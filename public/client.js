...
function HistoryModal({ roundHistory, players, mySeat, cumulative, onClose }) {
  ...
  const botSeats = [0, 1, 2, 3].filter(s => players[s] === null);
  function rowValues(row) {
    const rowPlayers = row.players || players;
    const usedIndices = new Set();
    const result = {};
    players.forEach((name, colSeat) => {
      if (!name) return;
      const idx = rowPlayers.indexOf(name);
      if (idx !== -1) { result[colSeat] = row.net[idx]; usedIndices.add(idx); }
    });
    const leftover = [0, 1, 2, 3].filter(i => !usedIndices.has(i));
    botSeats.forEach((colSeat, i) => {
      if (leftover[i] !== undefined) result[colSeat] = row.net[leftover[i]];
    });
    return result;
  }
  function valueFor(row, colSeat) {
    const v = rowValues(row)[colSeat];
